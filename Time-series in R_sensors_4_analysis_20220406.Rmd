...
A simple approach for removing deterministic trends and/or seasonal effects from a time series is by taking differences (i.e. a way to make a non-stationary time series stationary). A practical interpretation of taking differences is that by doing so, the changes in the data will be monitored, but no
longer the series itself.

We illustrate how differencing works by using the Mauna Loa CO2 Concentrations dataset original series:
...
adf.test(co2)
```
There is a clear trend, which is close to linear, thus the simple approach should work well here. Taking first-order differences with lag 1 shows the yearly changes in the rescues. In R, the job is done with function diff().
...
Testing for stationarity on the differenced data:
```{r ts.adf.test.1}
adf.test(d.co2)
```

Decomposing the TS again:
```{r ts.decomposition.1}
d.co2.components <- decompose(d.co2)
plot(d.co2.components)
```
...
An appealing idea for analyzing the correlation among consecutive observations in the above series is to produce a scatterplot of (x1,xt+1) for all t=1,....,n-1. There is a designated function `lag.plot()` in R. The result is as follows:
...
Now we will estimate the autocorrelation function (ACF) for any lag k . Here, we introduce the correlogram, the standard means of visualization for the ACF. We will then also study the properties of the ACF estimator. We employ R
and obtain:
...
The examples above clearly doesn’t show a quick drop in the correlations. 


```{r correlogram.plot.3}
acf(d.co2, ylim=c(-1,1))
```
